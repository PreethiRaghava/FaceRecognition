...
database_username_source = 'Raghava'
database_password_source = 'spsoft123'
database_ip_source      = '192.168.10.115'
...
##    fnl_Df = pd.read_csv("In Count Data.csv")
    df = pd.read_sql('SELECT * FROM employee_details.dataofemp;',con = database_connection_source)
    fnl_Df = pd.read_sql('SELECT * FROM employee_details.in_data;',con = database_connection_source)
...
##                     pass
                ...
                #print(matchname)
                 if type(matchname) == str and matchname != "Unknown":
                     ...
                     #print(a)
                     a.index = [0]
                     a["shift_time"],a["DATE"],a["IN TIME"],a["Day"] = "General",datetime.now().strftime("%d-%m-%Y"),datetime.now().strftime("%H:%M"), datetime.today().strftime('%A')
                     a.to_sql(name='in_data', con=database_connection_source, if_exists = 'append', index=False)
                     fnl_Df = pd.concat([fnl_Df,a],axis=0,join='outer',ignore_index=True)
##                     fnl_Df.to_csv("In Count Data.csv",index=False)
        ...
        #if cv2.waitKey(1) == 27:
            #break
        if face_reg == datetime.now().strftime("%H:%M:%S"):
            break
...
##dfn = pd.read_csv("SPsoft_face_marks.csv")
dfn = pd.read_sql('SELECT * FROM employee_details.SPsoft_face_marks;',con = database_connection_source)
nodal_points = []
